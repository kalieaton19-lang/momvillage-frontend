...
export default function ProfilePage() {
  ...
  async function handleToggleLike(postId: string) {
    ...
    try {
      ...
      const interactions = await fetchPostInteractions([postId], currentUser.id);
                  ...
                  <div className="flex items-start justify-between gap-3 mb-3">
                    {getProfileHref(post.author_user_id) ? (
                      <div className="flex items-center gap-3 min-w-0">
                        <Link href={getProfileHref(post.author_user_id)!} className="flex items-center gap-3 group w-fit max-w-full">
                        ...
                        <div className="min-w-0">
                          {post.group_id ? (
                            <div className="space-y-0.5 min-w-0">
                              <Link href={getProfileHref(post.author_user_id)!} className="font-semibold text-zinc-900 dark:text-zinc-50 group-hover:underline">{formatFirstNameLastInitial(authorNameById[post.author_user_id] || post.author_name || "Mom")}</Link>
                            ...
                            <div className="flex items-center gap-1 min-w-0 text-sm">
                              <Link href={getProfileHref(post.author_user_id)!} className="font-semibold text-zinc-900 dark:text-zinc-50 truncate group-hover:underline">{authorNameById[post.author_user_id] || post.author_name || "Mom"}</Link>
                            </div>
                          )}
                      ...
                      <div className="flex items-center gap-3 min-w-0">
                        ...
                        <div className="min-w-0">
                          {post.group_id ? (
                            <div className="space-y-0.5 min-w-0">
                              <div className="font-semibold text-zinc-900 dark:text-zinc-50">{formatFirstNameLastInitial(authorNameById[post.author_user_id] || post.author_name || "Mom")}</div>
                              <div className="text-xs text-pink-600 dark:text-pink-300 leading-tight break-words">
                                posted in <Link href={`/groups/${post.group_id}`} className="font-bold hover:text-pink-700 dark:hover:text-pink-200 break-words">{groupNameById[post.group_id] || "Group"}</Link>
                              </div>
                            </div>
