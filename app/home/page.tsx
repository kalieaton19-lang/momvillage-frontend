      ...
      if (currentUser?.user_metadata) {
        ...
        setProfile({
          ...
          profile_photo_url:
            ...
            currentUser.user_metadata?.profile_photo_url ||
            "",
        });
      }
    } catch (error) {
                ...
                >
                  ...
                  <div className="mt-3 pt-3 border-t border-zinc-200 dark:border-zinc-800 flex items-center gap-4 text-sm">
                    <button
                      ...
                      className={`inline-flex items-center gap-1 transition-colors disabled:opacity-60 ${likedByMeByPost[post.id] ? 'text-pink-600 dark:text-pink-300' : 'text-zinc-700 dark:text-zinc-300'}`}
                      aria-label={`Likes ${likesCountByPost[post.id] || 0}`}
                    >
                      <svg width="23" height="23" viewBox="0 0 24 24" fill={likedByMeByPost[post.id] ? 'currentColor' : 'none'} stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round" aria-hidden="true">
                        <path d="M20.84 4.61a5.5 5.5 0 0 0-7.78 0L12 5.67l-1.06-1.06a5.5 5.5 0 0 0-7.78 7.78l1.06 1.06L12 21.23l7.78-7.78 1.06-1.06a5.5 5.5 0 0 0 0-7.78z" />
                      </svg>
                      <span className="text-sm font-medium leading-none">{likesCountByPost[post.id] || 0}</span>
                    </button>
                    {!post.comments_disabled && (
                      <button
                        ...
                        className={`inline-flex items-center gap-1 transition-colors ${openCommentsByPost[post.id] ? 'text-pink-600 dark:text-pink-300' : 'text-zinc-700 dark:text-zinc-300'}`}
                      ...
                      >
                        ...
                        <span className="text-sm font-medium leading-none">{(commentsByPost[post.id] || []).length}</span>
                      </button>
                    )}
                    {post.visibility === 'public' && (
                      <button
                        ...
                        className="inline-flex items-center gap-1 text-zinc-700 dark:text-zinc-300 transition-colors disabled:opacity-60"
                      ...
                      >
                        ...
                        <span className="text-sm font-medium leading-none">{sharesCountByPost[post.id] || 0}</span>
                      </button>
                    )}
                  </div>
