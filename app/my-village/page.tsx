...
function InviteByNameForm({ onBack, onSelect }: { onBack: () => void; onSelect: (user: any) => void }) {
  ...
  // Fetch users for "My Village" (all users in the village)
  useEffect(() => {
  ...
  // Handle user selection from InviteByNameForm
  ...
  // Re-fetch invited users when the tab changes
  useEffect(() => {
  ...
  const handleResend = async (invitation: any) => {
    ...
    setSuccess("");
    try {
      // Logic to resend the invitation (e.g., call a Supabase function)
      const { error: resendError } = await supabase
        .from("invitations")
        .update({ status: "pending" }) // Assuming you update the status to resend
        .eq("id", invitation.id);
      if (resendError) throw resendError;
      setSuccess("Invitation resent successfully.");
      // Optionally, refetch invited users
  ...
  const handleCancel = async (invitation: any) => {
    ...
    setSuccess("");
    try {
      // Logic to cancel the invitation (e.g., call a Supabase function)
